    ...
    FileInput.prototype = {
                ...
                handler($el, 'click', function () {
                    ...
                    $frame.fadeOut('slow', function () {
                        ...
                        var filestack = self.getFileStack(true), len = filestack.length,
                            cGS = previewCache.count(self.id);
                        self.clearFileInput();
                        if (len === 0 && cGS === 0) {
                            self.reset();
                        } else {
        ...
        uploadSingle: function (i, files, allFiles) {
            var self = this, total = self.getFileStack().length, formdata = new FormData(), outData,
                ...
                pct, cGSComplete, $btnUpload = $thumb.find('.kv-file-upload'), $btnDelete = $thumb.find('.kv-file-remove'),
        ...
        previewFile: function (i, file, theFile, previewId, data) {
            ...
            var self = this, cat = self.parseFileType(file), caption = self.slug(file.name), content, strText,
                ...
                cGSTypes = types.indexOf(cat) >= 0, id, height,
                cGSMimes = isEmpty(mimes) || (!isEmpty(mimes) && mimes.indexOf(file.type) !== -1),
                footer = self.renderFileFooter(caption, config.width), modal = '',
                ind = previewId.slice(previewId.lastIndexOf('-') + 1);
            if (cGSTypes && cGSMimes) {
                if (cat === 'text') {
        ...
        readFiles: function (files) {
            ...
            function readFile(i) {
                ...
                var node = ctr + i, previewId = previewInitId + "-" + node, isText, file = files[i],
                    ...
                    previewData = objUrl.createObjectURL(file), fileCount = 0, j, msg, typ, cGS,
                ...
                if (!isEmpty(fileTypes) && isArray(fileTypes)) {
                    for (j = 0; j < fileTypes.length; j += 1) {
                        ...
                        cGS = (checkFile !== undefined && checkFile(file.type, caption));
                        fileCount += isEmpty(cGS) ? 0 : cGS.length;
                    }
                    if (fileCount === 0) {
        ...
        checkDimensions: function (i, cGS, $img, $thumb, fname, type, params) {
            var self = this, msg, dim, tag = cGS === 'Small' ? 'min' : 'max',
            ...
            isValid = cGS === 'Small' ? dim >= limit : dim <= limit;
            if (isValid) {
                return;
            }
            msg = self['msgImage' + type + cGS].replace('{name}', fname).replace('{size}', limit);
